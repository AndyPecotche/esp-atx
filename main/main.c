...
static void reconnectWG(void *arg) {
    ...
    while (1){
        ...
        if (err == ESP_OK) {
            ESP_LOGI(TAG, "Peer is STILL up");
            count = 0;
        } else {
            ESP_LOGI(TAG, "Peer is down NOW");
            count++;
            if (count > 10) {
                ESP_LOGE(TAG, "Peer is down, giving up, reconfiguring");
                    esp_restart();
                if (err != ESP_OK) {
...
void app_main()
{
    ...
    xTaskCreate(reconnectWG,"reconectWG", 4096, &ctx, 5, NULL);
}